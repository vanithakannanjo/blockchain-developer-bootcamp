	...
	describe('Withdrawing Tokens', () => {
		...
		describe('Success', () => {
			...
			it('withdraws token funds', async () => {
				expect(Number(await token1.balanceOf(exchange.address))).to.equal(Number(0))
				expect(Number(await exchange.tokens(token1.address, user1.address))).to.equal(Number(0))
				expect(Number(await exchange.balanceOf(token1.address, user1.address))).to.equal(Number(0))
			})
